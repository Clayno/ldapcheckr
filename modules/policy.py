...
class PassPol:
    def __init__(self, ldap_result, passpol_type="PSO"):
        ...
        if passpol_type == "PSO":
            ...
            self.description = (
                pol_attrs["description"] if "description" in pol_attrs.keys() else None
            )
            ...
            self.lockoutDuration = (
                convert_wi8_timestamp_to_timedelta(
                    pol_attrs["lockoutDuration"]
                ).total_seconds()
                / 60
            )
            self.lockoutObservationWindow = (
                convert_wi8_timestamp_to_timedelta(
                    pol_attrs["lockOutObservationWindow"]
                ).total_seconds()
                / 60
            )
            ...
            self.passwordComplexityEnabled = (
                True if "PASSWORD_COMPLEX" in flags else False
            )
            self.passwordReversibleEncryptionEnabled = (
                True if "PASSWORD_STORE_CLEARTEXT" in flags else False
            )
            self.PSOAppliesTo = []
...
class Policy(Module):
    async def _work(self):
        ...
        default_passpol = self.ldap_client.pagedsearch(
        ...
        # Iterator[(dict, Exception)]
        search = self.ldap_client.pagedsearch(
