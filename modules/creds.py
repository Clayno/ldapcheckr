...
INTERESTING_KEYWORDS = ["mdp", "mot de passe", "password", "passwd"]

INTERESTING_PATTERNS = ["[^\w]pwd[^\w]", "[^\w]pw[^\w]", "[^\w]mdp[^\w]"]
        ...
        async for user in users:
            ...
            for k, v in user["attributes"].items():
                ...
                if not isinstance(v, str):
                    continue
                if (
                    any(keyword in v.lower() for keyword in INTERESTING_KEYWORDS)
                    or (k in INTERESTING_ATTRIBUTES and INTERESTING_ATTRIBUTES[k])
                    or any(
                        re.match(pattern, v.lower()) for pattern in INTERESTING_PATTERNS
                    )
                    or any(
                        re.match(pattern, v.lower())
                        for pattern in INTERESTING_PATTERNS_IN_INTERESTING_ATTRIBUTES
